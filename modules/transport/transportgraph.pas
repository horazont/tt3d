...
uses
  Classes, SysUtils, Geometry, fgl, contnrs, GTBase, math;
...
const
  ...
  ARC_PATH_STEP_LENGTH = 0.1;
  BEZIER_PATH_STEP_LENGTH = 0.1;
  BEZIER_PATH_STEP_EPSILON = 0.001;
...
type
  ...
  public
    procedure DoChange;
  public
    property Node: TPathNode read FNode;
    property NodeLink: TPathNodeLink read FNodeLink;
  ...
  private
    ...
    FTo: TVector4;
  public
    property FromInfo: TVector4 read FFrom;
    property ToInfo: TVector4 read FTo;
    property GameLength: TVectorFloat read FLength;
  public
  ...
  private
    ...
    function GetSide(ASide: TPathNodeSideDirection): TPathLinkSide;
  ...
  public
    property Sides[ASide: TPathNodeSideDirection]: TPathLinkSide read GetSide;
  published
...
function SideOpposite(const ASide: TPathNodeSideDirection): TPathNodeSideDirection;
...
function SideOpposite(const ASide: TPathNodeSideDirection): TPathNodeSideDirection;
begin
  Exit(TPathNodeSideDirection(1-Ord(ASide)));
end;
...
function TPathLink.GetSide(ASide: TPathNodeSideDirection): TPathLinkSide;
begin
  Result := FSides[ASide];
end;
...
begin
  ...
  if FYaw = ClampedYaw then
    Exit;
  inherited SetYaw(ClampedYaw);
  UpdateSpeedLimits(FMaxRadius);
end;
...
begin
  ...
  t := tStep;
  vPrev := Vector4(vB, 0.0);
  while a < FAngleEnd do
  begin
    vTmp := BezierHeightYaw ** t;
    vCurr := Vector4(AngleToVec(a) * FRadius + FCenter, vTmp.X, vTmp.Y);

    AddPathSegment(vPrev, vCurr, ARC_PATH_STEP_LENGTH);
  ...
  begin
    ...
    AddPathSegment(vPrev, vCurr, Abs(a - aPrev) * FRadius);
  end;
